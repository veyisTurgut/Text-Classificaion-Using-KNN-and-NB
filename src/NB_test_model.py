...
MULTILABEL_THRESHOLD_MULTIPLIER = 5 ## found after running the algorithm several times
...
### TRAINING PHASE
# with %100 of the training data, prepare a word based vocab
counter = 0
        ...
        for word in words:
            if word not in VOCAB_WORD_BASED:
                ...
                for topic in topics:
                    ...
                    VOCAB_WORD_BASED[word][topic] += 1
print(counter)
# with %100 of the training data, prepare a topic based vocab
with open(os.getcwd()+'/input/NB_training.csv') as csvfile:    
    csvreader = csv.reader(csvfile, delimiter=',', quotechar='"')
    for row in csvreader:
        id = int(row[0])
...
with open(os.getcwd()+'/input/NB_test.csv') as csvfile:
    csvreader = csv.reader(csvfile, delimiter=',', quotechar='"')
    line_number = 0 
    for row in csvreader:
        id = int(row[0])
...
with open(os.getcwd()+'/output/NB_pred.csv', 'w') as test_file:
...
with open(os.getcwd()+'/output/NB_true.csv', 'w') as test_file:
    header = ['doc_id']+ TOPICS
    file_writer = csv.DictWriter(test_file, fieldnames = header)
    file_writer.writeheader()
